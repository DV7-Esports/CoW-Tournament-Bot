...
export default class CommandHandler {
    ...
    static async handleCommand(client: DiscordClient, message: Message) {
        ...
        if (cmd.info.require) {
            ...
            if (cmd.info.require.roles && !isUserDeveloper(client, message.author.id)) {
                const perms: string[] = [];
                cmd.info.require.roles.forEach(async role => {
                    if ((message.member as GuildMember).roles.cache.filter(user_role => user_role.id === role)) return;
                    else {
                        const role_name = await message.guild?.roles.fetch(role);
                        return perms.push(`\`${role_name?.name}\``);
                    }
                });
                if (perms.length)
                    return await message.channel.send({
                        embeds: [
                            {
                                ...
                                description: `${message.author}, you must have these roles to run this command.\n\n${perms.join('\n')}`
                            }
                        ]
                    });
            }
